...
const movies = [
  ...
  export default function MovieList() {
    ...
    <div>
        ...
        {movies.map((movie) => (
          <li key={movie.genre} className="flex justify-between gap-x-6 py-5 bg-white shadow-lg rounded-xl p-8 space-y-8 mb-6">
            ...
            <div className="mt-4 space-x-4">
              <button onClick={handleOpenInfo}  className="px-4 py-2 text-sm font-semibold text-indigo-600 border border-indigo-600 rounded-md hover:text-indigo-400 hover:border-indigo-400">
                Details
              </button>
      ...
      <Movieinfo isOpen={isInfoOpen} onClose={handleCloseInfo}/>
      </div>
    )
  }